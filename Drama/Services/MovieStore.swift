...
class MovieStore: MovieService {
    ...
    func fetchMovies(from endpoint: MoviesEndPoint, completion: @escaping (Result<MovieResponse, MovieError>) -> ()) {
    ...
    private func loadUrlAndDecode<D: Decodable>(url: URL, params: [String: String]? = nil, completion: @escaping (Result<D, MovieError>) -> ()){
        ...
        urlSession.dataTask(with: finalURL) {[weak self] data, response, error in
            ...
            do {
                let docodedResponse = try self.jsonDecoder.decode(D.self, from: data)
                self.executeCompletionHandlerInMainThread(with: .success(docodedResponse), completion: completion)
            } catch {
